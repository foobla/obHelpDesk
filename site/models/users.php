 ...
 * @since       1.6
 */
class obHelpDeskModelUsers extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getItems()
	{
		...
		$query = "SELECT `user_id` FROM `#__obhelpdesk3_staffs`";
		$db->setQuery($query);
		$staff = $db->loadColumn();
		...
		if (empty($this->cache[$store]))
		{
			...
			{
				$items1 = parent::getItems();
			}

			// Bail out on an error or empty list.
			if (empty($items1))
			{
				$this->cache[$store] = $items1;
