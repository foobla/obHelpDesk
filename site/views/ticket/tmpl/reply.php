...
<div class="obhelpdesk-profile-bar">
	...
	<div class="btn-group pull-right">
		<button class="btn btn-mini" id="show-ticket-detail-btn" title="<?php echo JText::_( 'OBHELPDESK_SHOW_TICKET_DETAILS' ) ?>" onclick="ShowDetailTicket();">
			<i id="icon-detail-ticket" class="icon-expand icon-white"></i>&nbsp;<?php echo JText::_( 'COM_OBHELPDESK_TICKET_DETAILS' ) ?>
		</button>
		...
		<ul class="dropdown-menu">
			<?php if ( $this->item->status != 'closed' ): ?>
				<?php if ( $this->perm_update ): ?>
					<li>
						<a class="" title="<?php echo JText::_( 'COM_OBHELPDESK_CLOSE_THIS_TICKET' ) ?>" onclick="return obHelpDeskValidate('ticket.close')"><i class="icon-lock icon-white"></i>&nbsp;<?php echo JText::_( 'COM_OBHELPDESK_CLOSE_THIS_TICKET' ) ?>
						</a>
					</li>
				<?php endif; ?>
			...
			<?php else: ?>
				<li>
					<a class="" title="<?php echo JText::_( 'COM_OBHELPDESK_REOPEN_THIS_TICKET' ) ?>" onclick="return obHelpDeskValidate('ticket.reopen')"><i class="icon-undo icon-white"></i>&nbsp;<?php echo JText::_( 'COM_OBHELPDESK_REOPEN_THIS_TICKET' ) ?>
					</a>
				</li>
			<?php endif; ?>
			...
			<?php if ( $this->perm_delete ): ?>
				<li>
					<a class="" title="<?php echo JText::_( 'COM_OBHELPDESK_DELETE_THIS_TICKET' ) ?>" onclick="return obHelpDeskValidate('ticket.remove')"><i class="icon-remove icon-white"></i>&nbsp;<?php echo JText::_( 'COM_OBHELPDESK_DELETE_THIS_TICKET' ) ?>
					</a>
				</li>
			<?php endif; ?>
		</ul>
...
<table class="table table-bordered" id="obhelpdesk-ticket-detail" style="display:none;">
		...
		foreach ( $this->fields as $field ) :
			...
			<tr class="customfields">
				<td style="text-align: right;">
					<label id="jform_obhelpdesk_<?php echo $field->id ?>-lbl" for="jform_obhelpdesk_<?php echo $field->id ?>" class="hasTip" title="<?php echo $field->title; ?>::<?php echo $field->helptext; ?>"><?php echo $field->title;
						if ( $field->required ) {
							echo '<code>*</code>';
						}
						?>
	...
	<tr>
		<td style="text-align:right">
			<label id="jform_obhelpdesk_subject" for="jform_obhelpdesk_subject"><?php echo JText::_( 'COM_OBHELPDESK_SUBJECT' ); ?>
				<code>*</code></label></td>
		<td><?php echo $this->form->getInput( 'subject', null, $this->item->subject ); ?></td>
	</tr>
	<tr>
		<td style="text-align:right">
			<label id="jform_obhelpdesk_staff" for="jform_obhelpdesk_staff"><?php echo JText::_( 'COM_OBHELPDESK_STAFF' ); ?></label>
		</td>
		<td><?php if ( $this->listAssignee ) {
				echo $this->listAssignee;
			} else {
				echo JText::_( 'OBHELPDESK_UNASSIGNED' );
			} ?>
	</tr>
	<tr>
		<td style="text-align:right">
			<label id="jform_obhelpdesk_staff" for="jform_obhelpdesk_staff"><?php echo JText::_( 'COM_OBHELPDESK_PRIORITY' ); ?></label>
		</td>
		<td><?php if ( $this->listPriority ) {
				echo $this->listPriority;
			} ?>
	</tr>
	<tr>
		<td style="text-align:right">
			<label id="jform_obhelpdesk_staff" for="jform_obhelpdesk_staff"><?php echo JText::_( 'COM_OBHELPDESK_STATUS' ); ?></label>
		</td>
		<td><?php if ( $this->listStatus ) {
	...
	<?php if ( $this->item->status != 'closed' ) : ?>
		<?php if ( $this->updatePerm ): ?>
			<tr>
				<td colspan="2">
					<button type="button" class="btn btn-small btn-primary" onClick="return Joomla.submitbutton('ticket.update')">
						<i class="icon-ok icon-white"></i> <?php echo JText::_( 'COM_OBHELPDESK_UPDATE' ) ?></button>
				</td>
			</tr>
		<?php endif; ?>
	<?php endif; ?>
					...
					<div class="btn-group">
						...
						<ul class="dropdown-menu">
							<?php if ( $this->is_staff ) : ?>
								<li>
									<a href="#" onclick="Joomla.submitbutton('ticket.replyopen');return false;"><i class="icon-checkbox-unchecked icon-white"></i> <?php echo JText::_( 'OBHELPDESK_REPLY_OPEN' ); ?>
									</a></li>
							<?php else: ?>
								<li>
									<a href="#" onclick="Joomla.submitbutton('ticket.replyonhold');return false;"><i class="icon-checkbox icon-white"></i> <?php echo JText::_( 'OBHELPDESK_REPLY_HOLD' ); ?>
									</a></li>
							<?php endif; ?>
							<li>
								<a href="#" onclick="Joomla.submitbutton('ticket.replyclose');return false;"><i class="icon-locked icon-white"></i> <?php echo JText::_( 'OBHELPDESK_REPLY_CLOSE' ); ?>
								</a></li>
						</ul>
					</div>
				</td>
